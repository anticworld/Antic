 ...
 */
package org.antic.consensus;

import static org.antic.core.Fork.UNIFORM_DISTRIBUTION;
...
import org.antic.consensus.AnticBft.Event.Type;
import org.antic.consensus.exception.AnticBftException;
...
import org.antic.core.TransactionExecutor;
...
import org.antic.crypto.Key.Signature;
import org.antic.net.Channel;
import org.antic.net.ChannelManager;
...
import org.antic.net.msg.consensus.NewViewMessage;
import org.antic.net.msg.consensus.ProposalMessage;
import org.antic.net.msg.consensus.VoteMessage;
import org.antic.util.ArrayUtil;
import org.antic.util.ByteArray;
import org.antic.util.Bytes;
import org.antic.util.MerkleUtil;
import org.antic.util.SystemUtil;
import org.antic.util.TimeUtil;
import org.antic.vm.client.AnticBlock;
import org.antic.vm.client.AnticBlockStore;
...
/**
 * Implements Antic BFT engine based on single-thread event model. States are
 ...
 * </ul>
 */
public class AnticBft implements BftManager {
    private static final Logger logger = LoggerFactory.getLogger(AnticBft.class);
    ...
    public AnticBft(Kernel kernel) {
        this.kernel = kernel;
        this.config = kernel.getConfig();

        this.chain = kernel.getBlockchain();
        this.blockStore = new AnticBlockStore(chain);
    ...
    public void start() {
        if (status == Status.STOPPED) {
            ...
            timer.start();
            broadcaster.start();
            logger.info("Antic BFT manager started");

            enterNewHeight();
            eventLoop();

            logger.info("Antic BFT manager stopped");
        }
    }
    ...
    protected void enterCommit() {
        ...
        if (!blockHash.isPresent()) {
            throw new AnticBftException("Entered COMMIT state without +2/3 pre-commit votes");
        } else {
    ...
    protected Block proposeBlock() {
        ...
        AnticBlock anticBlock = new AnticBlock(tempHeader, config.spec().maxBlockGasLimit());
        ...
        for (PendingManager.PendingTransaction pendingTx : pendingTxs) {
            ...
            TransactionResult result = exec.execute(tx, asTrack, dsTrack, anticBlock, gasUsedInBlock);
    ...
    protected boolean validateBlockProposal(BlockHeader header, List<Transaction> transactions) {
        try {
            ...
            List<TransactionResult> results = transactionExecutor.execute(transactions, asTrack, dsTrack,
                    new AnticBlock(header, config.spec().maxBlockGasLimit()), 0);
