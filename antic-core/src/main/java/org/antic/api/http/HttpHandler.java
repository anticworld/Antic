...
import org.antic.api.ApiHandler;
import org.antic.config.Config;
import org.antic.util.BasicAuth;
...
public class HttpHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    private static final MimetypesFileTypeMap mimeTypesMap = new MimetypesFileTypeMap(
            HttpHandler.class.getResourceAsStream("/org/antic/api/mime.types"));
    ...
    protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest msg) {
        ...
        if ("/".equals(path)) {
            lastContentFuture = writeStaticFile(ctx, "/org/antic/api/index.html");
            ...
            if (path.startsWith("/swagger-ui/")) {
                ...
                lastContentFuture = writeStaticFile(ctx, "/org/antic/api" + path);
            }
            ...
            if (apiHandler.isAuthRequired(msg.method(), path) && !checkBasicAuth(headers)) {
                ...
                resp.headers().set(HttpHeaderNames.WWW_AUTHENTICATE, "Basic realm=\"Antic RESTful API\"");
