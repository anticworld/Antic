#include "org_antic_crypto_Native.h"
#include "ripemd160.h"
...
JNIEXPORT jbyteArray JNICALL Java_org_antic_crypto_Native_h256
(JNIEnv *env, jclass cls, jbyteArray msg)
{
    // check inputs
    if (msg == NULL) {
        env->ThrowNew(env->FindClass("org/antic/crypto/CryptoException"), "Input can't be null");
        return NULL;
    }
...
JNIEXPORT jbyteArray JNICALL Java_org_antic_crypto_Native_h160
(JNIEnv *env, jclass cls, jbyteArray msg)
{
    // check inputs
    if (msg == NULL) {
...
JNIEXPORT jbyteArray JNICALL Java_org_antic_crypto_Native_sign
...
JNIEXPORT jboolean JNICALL Java_org_antic_crypto_Native_verify
...
JNIEXPORT jboolean JNICALL Java_org_antic_crypto_Native_verifyBatch
(JNIEnv *env, jclass cls, jobjectArray msgs, jobjectArray sigs, jobjectArray pks)
{
    ...
    if (msgs_size != sigs_size || sigs_size != pks_size) {
        env->ThrowNew(env->FindClass("org/antic/crypto/CryptoException"), "Input arrays must have an identical length");
        return false;
    }
